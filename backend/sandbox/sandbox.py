...
# Check if we should use local containers or Daytona
if config.USE_LOCAL_CONTAINERS:
    logger.info("Using local Docker containers for sandbox management")
    # Import local sandbox implementation
    from sandbox.local_sandbox import (
        get_or_start_sandbox as local_get_or_start_sandbox,
        create_sandbox as local_create_sandbox,
        delete_sandbox as local_delete_sandbox,
        start_supervisord_session as local_start_supervisord_session
    )
    
    # Use local implementations
    get_or_start_sandbox = local_get_or_start_sandbox
    create_sandbox = local_create_sandbox
    delete_sandbox = local_delete_sandbox
    start_supervisord_session = local_start_supervisord_session
    
else:
    logger.info("Using Daytona for sandbox management")
    # Import and configure Daytona
...
# Only define Daytona functions if not using local containers
if not config.USE_LOCAL_CONTAINERS:
